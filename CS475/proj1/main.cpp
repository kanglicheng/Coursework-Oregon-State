...
int main(int argc, char *argv[]) {
  ...
  // thread timing: fill the random-value arrays:
...
#pragma omp parallel for default(none) shared(BeforeY, AfterY, DistX, stderr) reduction(+:numSuccesses)
    for (int n = 0; n < NUMTRIALS; n++) {
      ...
      float vx = sqrt(2.0 * GRAVITY * (beforey - aftery));
      // float t = sqrt((2.0 * (beforey - aftery)) / GRAVITY));
      float t = sqrt(((2.0 * aftery)) / GRAVITY);
      float dx = vx * t;
      if (dx - distx <= RADIUS)
        numSuccesses++;
  ...
  float probability = (float)numSuccesses /
                      (float)(NUMTRIALS); // just get for last NUMTIMES run
